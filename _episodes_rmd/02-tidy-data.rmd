...
app or machine-learning model, oompletely via code. Depending on the complexity of the data and the analysis, this may be made up of a series of different coding
...
with the actual analysis part only occuring at the end.
...
In R, the prefered format is 'long', where one column contains all of the values with the remaining columns giving those columns context. Below is
an example of data in a wide format,

![alt text](https://github.com/RobHarrand/life-sciences-spreadsheet-best-practice/blob/gh-pages/fig/08-wide-data.png "Wide data")

Here is the same data restructued in a long format,

![alt text](https://github.com/RobHarrand/life-sciences-spreadsheet-best-practice/blob/gh-pages/fig/09-long-data.png "Long data")

This format, where each row corresponds to a single data point or observation, is also know as 'tidy data', and is the prefered format in R. The goal
...
the ELISA plate. It doesn't look 'untidy' as that word is commonly used, but what you've actually got is a grid of measurements, often multiple
measurements for a given sample. Once you add these definitions to each plate file, the 'tidy' version becomes apparent.

There are also points during analysis where you'll have intermediate datasets, where again, tidy datasets will be benefical.
